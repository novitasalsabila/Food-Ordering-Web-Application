...
// Testing untuk membuka halaman kontak
describe("GET /contact", () => {
  test("should return the contact page", async () => {
    const response = await request(app).get("/contact").set("Cookie", [
      "cookuid=1; cookuname=User",
    ]);
    expect(response.status).toBe(200);
    expect(response.text).toContain("Contact Us"); // Pastikan halaman memuat konten kontak
  });
});